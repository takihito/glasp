...
> **注意:** `go install` では OAuth 認証情報がバイナリに埋め込まれません。実行時に `GLASP_CLIENT_ID` と `GLASP_CLIENT_SECRET` 環境変数の設定が必要です。Releases ページのビルド済みバイナリには認証情報が埋め込まれているため、この手順は不要です。
...
> **Windows:** `.tar.gz` の代わりに `.zip` アーカイブをダウンロードしてください。

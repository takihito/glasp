...
GAS（Google Apps Script）コードをローカルで開発・管理・デプロイできる CLIツールです。

Node.js ベースの [clasp](https://github.com/google/clasp) を置き換えるシングルバイナリの代替ツールで、clasp との完全な互換性を備えています。
...
- ローカルで認証情報を取得します
  ...
  ```
- repository secret `GLASP_AUTH` として登録します（**Settings → Secrets and variables → Actions**）

...
Because glasp is a single precompiled Go binary, setup in GitHub Actions is dramatically faster than installing clasp via npm. The table below shows benchmark results measured on `ubuntu-latest` (glasp v0.3.0 / @google/clasp 3.3.0, push/pull averaged over 5 runs):
...
```yaml
- uses: takihito/glasp@v0.3.0   # recommended: pin to a release tag
  with:
    version: 'v0.3.0'
```

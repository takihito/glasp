...
Because glasp is a single precompiled Go binary, setup in GitHub Actions is dramatically faster than installing clasp via npm. The table below shows benchmark results measured on `ubuntu-latest` (glasp v0.2.10 / @google/clasp 3.3.0, push/pull averaged over 5 runs):
...
```yaml
- uses: takihito/glasp@v0.2.10   # recommended: pin to a release tag
  with:
    version: 'v0.2.10'
```
...
```yaml
- uses: takihito/glasp@9eebd88f080893412c59dfac97a7af527aaddbb5 # v0.2.10
  with:
    version: 'v0.2.10'
```
...
- OAuth2 クライアントクレデンシャル（`GLASP_CLIENT_ID`, `GLASP_CLIENT_SECRET`）を使用する場合は、これらも環境変数としてエクスポートされます。これにより、glasp の認証フローでクライアントクレデンシャルが利用されます。GitHub Actions 内でこれらの値を `client-id` と `client-secret` を入力パラメータとして指定することでOAuth2 認証に必要なクレデンシャルを安全に管理できます。

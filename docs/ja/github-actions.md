...
| `auth` | いいえ | | `.clasprc.json` の JSON 内容。リポジトリシークレットを指定します。設定すると後続のステップで `GLASP_AUTH` 環境変数として使用できます。 |
...
| `client-id` | いいえ | | OAuth2 クライアント ID。設定すると `GLASP_CLIENT_ID` 環境変数にエクスポートされます。 |
| `client-secret` | いいえ | | OAuth2 クライアントシークレット。設定すると `GLASP_CLIENT_SECRET` 環境変数にエクスポートされます。 |
...
- uses: takihito/glasp@v0.2.7
  with:
    ...
    glasp_client_id: ${{ secrets.GLASP_CLIENT_ID }}         # OAuth2 クライアントIDを指定する場合
    glasp_client_secret: ${{ secrets.GLASP_CLIENT_SECRET }} # OAuth2 クライアントシークレットを指定する場合
...
環境変数 `GLASP_AUTH` を使用し、JSON 内容をそのまま認証情報として使用します。ファイルへの書き込みや `glasp login` の実行は不要です。glasp 内部の認証ソースの優先順位：

-  `--auth <path>` フラグ
-  `GLASP_AUTH` 環境変数
   -  `.glasp/access.json`ファイルの内容
   -  `~/.clasprc.json`ファイルの内容 
- OAuth2 クライアントクレデンシャル（`GLASP_CLIENT_ID`, `GLASP_CLIENT_SECRET`）を使用する場合は、これらも環境変数としてエクスポートされます。これにより、glasp の認証フローでクライアントクレデンシャルが利用されます。GitHub Actions 内でこれらの値を `glasp_client_id`  と `glasp_client_secret` を入力パラメータとして指定することでOAuth2 認証に必要なクレデンシャルを安全に管理できます。

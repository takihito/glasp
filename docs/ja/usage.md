...
`push` と `convert` は既定で、ローカルファイル収集時に**シンボリックリンクされたファイルをスキップ**します（リンク先を読み込みません）。これは、他のユーザーやツールと共有しているプロジェクトディレクトリなどで、プロジェクト外を指すシンボリックリンク経由のファイルを誤ってアップロードしてしまうことを防ぐためです。

`--allow-symlinks`（または `GLASP_ALLOW_SYMLINKS=1`）を指定すると、シンボリックリンクされたファイルを辿るようになります。このフラグを指定した場合でも、リンク先が `rootDir` の外、または認証トークンなど内部データを保持する `.glasp/` 配下を指しているシンボリックリンクはエラーとして拒否され、無条件に辿られることはありません。シンボリックリンクされた**ディレクトリ**は、フラグの有無にかかわらず辿られません。

```bash
# push 時にシンボリックリンクされたファイルを辿る
...
ファイルがスキップされた場合（`.claspignore` によるもの、`.d.ts` 宣言ファイル、シンボリックリンク）、`push`/`convert` は `Skipped 2 file(s): 1 ignored, 1 symlink` のような1行のサマリーを表示します。ファイルごとの詳細は `--log-level debug` で確認できます。

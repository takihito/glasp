...
`--allow-symlinks`（または `GLASP_ALLOW_SYMLINKS=1`）を指定すると、シンボリックリンクされたファイルを辿るようになります。このフラグを指定した場合でも、リンク先が `rootDir` の外を指しているシンボリックリンクはエラーとして拒否され、無条件に辿られることはありません。シンボリックリンクされた**ディレクトリ**は、フラグの有無にかかわらず辿られません。
